...
fun_input_bind_port(){
    ...
    echo ""
    echo -n -e "请输入 ${program_name} ${COLOR_GREEN}绑定端口${COLOR_END} [1-65535]"
...
fun_input_dashboard_port(){
    ...
    echo ""
    echo -n -e "请输入 ${program_name} ${COLOR_GREEN}管理端口${COLOR_END} [1-65535]"
...
fun_input_vhost_http_port(){
    ...
    echo ""
    echo -n -e "请输入 ${program_name} ${COLOR_GREEN}HTTP 请求的监听端口${COLOR_END} [1-65535]"
...
fun_input_vhost_https_port(){
    ...
    echo ""
    echo -n -e "请输入 ${program_name} ${COLOR_GREEN}HTTPS 请求的监听端口${COLOR_END} [1-65535]"
...
fun_input_log_max_days(){
    ...
    def_log_max_days="3"
    echo ""
    echo -e "请输入 ${program_name} ${COLOR_GREEN}日志最大天数${COLOR_END} [1-${def_max_days}]"
...
fun_input_max_pool_count(){
    ...
    echo ""
    echo -e "请输入 ${program_name} ${COLOR_GREEN}最大池计数${COLOR_END} [1-${def_max_pool}]"
...
fun_input_dashboard_user(){
    ...
    echo ""
    echo -n -e "请输入 ${program_name} ${COLOR_GREEN}管理用户名${COLOR_END}"
    read -e -p "(默认用户名 : ${def_dashboard_user}):" input_dashboard_user
    [ -z "${input_dashboard_user}" ] && input_dashboard_user="${def_dashboard_user}"
}
fun_input_dashboard_pwd(){
    def_dashboard_pwd=`fun_randstr 8`
    echo ""
    echo -n -e "请输入 ${program_name} ${COLOR_GREEN}管理密码${COLOR_END}"
    read -e -p "(默认密码 : ${def_dashboard_pwd}):" input_dashboard_pwd
    [ -z "${input_dashboard_pwd}" ] && input_dashboard_pwd="${def_dashboard_pwd}"
}
fun_input_token(){
    def_token=`fun_randstr 16`
    echo ""
    echo -n -e "请输入 ${program_name} ${COLOR_GREEN}令牌${COLOR_END}"
    read -e -p "(默认令牌 : ${def_token}):" input_token
    [ -z "${input_token}" ] && input_token="${def_token}"
}
fun_input_subdomain_host(){
    def_subdomain_host=${defIP}
    echo ""
    echo -n -e "请输入 ${program_name} ${COLOR_GREEN}子域名${COLOR_END}"
    read -e -p "(默认无域名 : ${def_subdomain_host}):" input_subdomain_host
    [ -z "${input_subdomain_host}" ] && input_subdomain_host="${def_subdomain_host}"
}

pre_install_clang(){
    fun_clangcn
    echo -e "请检查您的服务器设置，请稍候..."
    disable_selinux
    if [ -s ${str_program_dir}/${program_name} ] && [ -s ${program_init} ]; then
        ...
        echo -e "${program_name} 绑定端口: ${COLOR_YELOW}${set_bind_port}${COLOR_END}"
        echo -e ""
        fun_input_vhost_http_port
        [ -n "${input_port}" ] && set_vhost_http_port="${input_port}"
        echo -e "${program_name} HTTP 请求的监听端口: ${COLOR_YELOW}${set_vhost_http_port}${COLOR_END}"
        echo -e ""
        fun_input_vhost_https_port
        [ -n "${input_port}" ] && set_vhost_https_port="${input_port}"
        echo -e "${program_name} HTTPS 请求的监听端口: ${COLOR_YELOW}${set_vhost_https_port}${COLOR_END}"
        echo -e ""
        fun_input_dashboard_port
        [ -n "${input_port}" ] && set_dashboard_port="${input_port}"
        echo -e "${program_name} 管理端口: ${COLOR_YELOW}${set_dashboard_port}${COLOR_END}"
        echo -e ""
        fun_input_dashboard_user
        [ -n "${input_dashboard_user}" ] && set_dashboard_user="${input_dashboard_user}"
        echo -e "${program_name} 管理员: ${COLOR_YELOW}${set_dashboard_user}${COLOR_END}"
        echo -e ""
        fun_input_dashboard_pwd
        [ -n "${input_dashboard_pwd}" ] && set_dashboard_pwd="${input_dashboard_pwd}"
        echo -e "${program_name} 管理密码: ${COLOR_YELOW}${set_dashboard_pwd}${COLOR_END}"
        echo -e ""
        fun_input_token
        [ -n "${input_token}" ] && set_token="${input_token}"
        echo -e "${program_name} 令牌: ${COLOR_YELOW}${set_token}${COLOR_END}"
        echo -e ""
        fun_input_subdomain_host
        [ -n "${input_subdomain_host}" ] && set_subdomain_host="${input_subdomain_host}"
        echo -e "${program_name} 子域名: ${COLOR_YELOW}${set_subdomain_host}${COLOR_END}"
        echo -e ""
        fun_input_max_pool_count
        [ -n "${input_number}" ] && set_max_pool_count="${input_number}"
        echo -e "${program_name} 最大池计数: ${COLOR_YELOW}${set_max_pool_count}${COLOR_END}"
        echo -e ""
        echo -e "请选择 ${COLOR_GREEN}日志等级${COLOR_END}"
        echo    "1: info (default)"
        echo    "2: warn"
        echo    "3: error"
        echo    "4: debug"    
        ...
        echo -e "${program_name} 日志最大天数: ${COLOR_YELOW}${set_log_max_days}${COLOR_END}"
        echo -e ""
        echo -e "请选择 ${COLOR_GREEN}日志文件${COLOR_END}"
        ...
        echo -e ""
        echo -e "请选择 ${COLOR_GREEN}kcp 支持${COLOR_END}"
        echo    "1: enable (default)"
        echo    "2: disable"
        echo "-------------------------"  
...
# 如果要通过仪表板配置或重新加载 frps，必须设置仪表板端口
...
# auth token
token = ${set_token}
# 当多人使用一个frps服务器时，对http、https类型配置子域名比较方便。
subdomain_host = ${set_subdomain_host}
# 只允许 frpc 绑定你列出的端口，如果你不设置任何内容，则没有任何限制
#allow_ports = 1-65535
